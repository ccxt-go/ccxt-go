...
func (this *Eqonex) FetchMarkets(goArgs ...*Variant) *Variant {
	...
	_ = params
	request := MkMap(&VarMap{"verbose": MkBool(true)})
	_ = request
	response := this.Call(MkString("publicGetGetInstrumentPairs"), this.Extend(request, params))
	_ = response
	instrumentPairs := this.SafeValue(response, MkString("instrumentPairs"), MkArray(&VarArray{}))
	_ = instrumentPairs
	markets := MkArray(&VarArray{})
	_ = markets
	for i := MkInteger(0); IsTrue(OpLw(i, instrumentPairs.Length)); OpIncr(&i) {
		market := this.ParseMarket(*(instrumentPairs).At(i))
		_ = market
		markets.Push(market)
	}
	return markets
...
func (this *Eqonex) ParseMarket(goArgs ...*Variant) *Variant {
	...
	_ = id
	uppercaseId := this.SafeString(market, MkString("symbol"))
	_ = uppercaseId
	assetType := this.SafeString(market, MkString("assetType"))
	_ = assetType
	spot := OpEq2(assetType, MkString("PAIR"))
	_ = spot
	swap := OpEq2(assetType, MkString("PERPETUAL_SWAP"))
	_ = swap
	type_ := OpTrinary(swap, MkString("swap"), MkString("spot"))
	_ = type_
	baseId := this.SafeString(market, MkString("currency"))
	_ = baseId
	quoteId := this.SafeString(market, MkString("contAmtCurr"))
	_ = quoteId
	base := this.SafeCurrencyCode(baseId)
	_ = base
	quote := this.SafeCurrencyCode(quoteId)
	_ = quote
	symbol := OpTrinary(swap, uppercaseId, OpAdd(base, OpAdd(MkString("/"), quote)))
	_ = symbol
	status := this.SafeInteger(market, MkString("securityStatus"))
	_ = status
	active := OpEq2(status, MkInteger(1))
	_ = active
...
func (this *Eqonex) FetchCurrencies(goArgs ...*Variant) *Variant {
	params := GoGetArg(goArgs, 0, MkMap(&VarMap{}))
	_ = params
	response := this.Call(MkString("publicGetGetInstruments"), params)
	_ = response
	currencies := MkMap(&VarMap{})
	_ = currencies
	instruments := this.SafeValue(response, MkString("instruments"), MkArray(&VarArray{}))
	_ = instruments
	for i := MkInteger(0); IsTrue(OpLw(i, instruments.Length)); OpIncr(&i) {
		currency := this.ParseCurrency(*(instruments).At(i))
		_ = currency
...
func (this *Eqonex) ParseCurrency(goArgs ...*Variant) *Variant {
	...
	_ = currency
	id := this.SafeString(currency, MkInteger(0))
	_ = id
	uppercaseId := this.SafeString(currency, MkInteger(1))
	_ = uppercaseId
	code := this.SafeCurrencyCode(uppercaseId)
	_ = code
	priceScale := this.SafeInteger(currency, MkInteger(2))
	_ = priceScale
	amountScale := this.SafeInteger(currency, MkInteger(3))
	_ = amountScale
	precision := Math.Max(priceScale, amountScale)
	_ = precision
	name := this.SafeString(currency, MkInteger(6))
	_ = name
	status := this.SafeInteger(currency, MkInteger(4))
	_ = status
	active := OpEq2(status, MkInteger(1))
	_ = active
	fee := this.SafeNumber(currency, MkInteger(5))
	_ = fee
	return MkMap(&VarMap{
		"id":          id,
		"info":        currency,
...
func (this *Eqonex) FetchOHLCV(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("publicGetGetChart"), this.Extend(request, params))
	_ = response
	chart := this.SafeValue(response, MkString("chart"), MkArray(&VarArray{}))
	_ = chart
...
func (this *Eqonex) ParseOHLCV(goArgs ...*Variant) *Variant {
	...
	_ = market
	timestamp := this.SafeInteger(ohlcv, MkInteger(0))
	_ = timestamp
	open := this.ParseNumber(this.ConvertFromScale(this.SafeString(ohlcv, MkInteger(1)), *(*(market).At(MkString("precision"))).At(MkString("price"))))
	_ = open
	high := this.ParseNumber(this.ConvertFromScale(this.SafeString(ohlcv, MkInteger(2)), *(*(market).At(MkString("precision"))).At(MkString("price"))))
	_ = high
	low := this.ParseNumber(this.ConvertFromScale(this.SafeString(ohlcv, MkInteger(3)), *(*(market).At(MkString("precision"))).At(MkString("price"))))
	_ = low
	close := this.ParseNumber(this.ConvertFromScale(this.SafeString(ohlcv, MkInteger(4)), *(*(market).At(MkString("precision"))).At(MkString("price"))))
	_ = close
	volume := this.ParseNumber(this.ConvertFromScale(this.SafeString(ohlcv, MkInteger(5)), *(*(market).At(MkString("precision"))).At(MkString("amount"))))
	_ = volume
...
func (this *Eqonex) ParseBidAsk(goArgs ...*Variant) *Variant {
	...
	_ = amountKey
	market := GoGetArg(goArgs, 3, MkUndefined())
	_ = market
	...
	priceString := this.SafeString(bidask, priceKey)
	_ = priceString
	amountString := this.SafeString(bidask, amountKey)
	_ = amountString
	return MkArray(&VarArray{
...
func (this *Eqonex) ParseOrderBook(goArgs ...*Variant) *Variant {
	...
	_ = amountKey
	market := GoGetArg(goArgs, 7, MkUndefined())
	_ = market
	result := MkMap(&VarMap{
		"timestamp": timestamp,
	...
	for i := MkInteger(0); IsTrue(OpLw(i, sides.Length)); OpIncr(&i) {
		...
		_ = orders
		bidasks := this.SafeValue(orderbook, side)
		_ = bidasks
...
func (this *Eqonex) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	market := this.Market(symbol)
	_ = market
	request := MkMap(&VarMap{"pairId": parseInt(*(market).At(MkString("id")))})
	_ = request
...
func (this *Eqonex) FetchTrades(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"pairId": parseInt(*(market).At(MkString("id")))})
	_ = request
	response := this.Call(MkString("publicGetGetTradeHistory"), this.Extend(request, params))
	_ = response
...
func (this *Eqonex) ParseTrade(goArgs ...*Variant) *Variant {
	...
	if IsTrue(GoIsArray(trade)) {
		...
		takerSide := this.SafeInteger(trade, MkInteger(4))
		_ = takerSide
		...
		if IsTrue(OpNotEq2(feeCost, MkUndefined())) {
			...
			feeCurrencyId := this.SafeString(trade, MkString("commCurrency"))
			_ = feeCurrencyId
...
func (this *Eqonex) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("privatePostGetPositions"), params)
	_ = response
	positions := this.SafeValue(response, MkString("positions"), MkArray(&VarArray{}))
	_ = positions
	...
	for i := MkInteger(0); IsTrue(OpLw(i, positions.Length)); OpIncr(&i) {
		...
		_ = position
		assetType := this.SafeString(position, MkString("assetType"))
		_ = assetType
		if IsTrue(OpEq2(assetType, MkString("ASSET"))) {
			currencyId := this.SafeString(position, MkString("symbol"))
			_ = currencyId
			code := this.SafeCurrencyCode(currencyId)
			_ = code
			quantityString := this.SafeString(position, MkString("quantity"))
			_ = quantityString
			availableQuantityString := this.SafeString(position, MkString("availableQuantity"))
			_ = availableQuantityString
			scale := this.SafeInteger(position, MkString("quantity_scale"))
			_ = scale
...
func (this *Eqonex) CreateOrder(goArgs ...*Variant) *Variant {
	...
	_ = orderSide
	quantityScale := this.GetScale(amount)
	_ = quantityScale
	request := MkMap(&VarMap{
		...
		*(request).At(MkString("ordType")) = MkInteger(1)
		...
		if IsTrue(OpEq2(type_, MkString("limit"))) {
			*(request).At(MkString("ordType")) = MkInteger(2)
			priceScale := this.GetScale(price)
			_ = priceScale
			...
			*(request).At(MkString("priceScale")) = OpCopy(priceScale)
			...
			stopPrice := this.SafeNumber2(params, MkString("stopPrice"), MkString("stopPx"))
			_ = stopPrice
					...
					*(request).At(MkString("stopPx")) = this.ConvertToScale(this.NumberToString(stopPrice), this.GetScale(stopPrice))
					...
					if IsTrue(OpEq2(type_, MkString("stop limit"))) {
						*(request).At(MkString("ordType")) = MkInteger(4)
						priceScale := this.GetScale(price)
						_ = priceScale
						stopPriceScale := this.GetScale(stopPrice)
						_ = stopPriceScale
...
func (this *Eqonex) FetchOrder(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	request := MkMap(&VarMap{"orderId": parseInt(id)})
	_ = request
	response := this.Call(MkString("privatePostGetOrderStatus"), this.Extend(request, params))
	_ = response
...
func (this *Eqonex) FetchClosedOrders(goArgs ...*Variant) *Variant {
	...
	params := GoGetArg(goArgs, 3, MkMap(&VarMap{}))
	_ = params
	request := MkMap(&VarMap{"ordStatus": MkString("2")})
	_ = request
...
func (this *Eqonex) FetchCanceledOrders(goArgs ...*Variant) *Variant {
	...
	params := GoGetArg(goArgs, 3, MkMap(&VarMap{}))
	_ = params
	request := MkMap(&VarMap{"ordStatus": MkString("4")})
	_ = request
	return this.FetchOrders(symbol, since, limit, this.Extend(request, params))
}
...
func (this *Eqonex) FetchOrders(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privatePostGetOrders"), this.Extend(request, params))
	_ = response
...
func (this *Eqonex) FetchDepositAddress(goArgs ...*Variant) *Variant {
	...
	_ = currency
	request := MkMap(&VarMap{"instrumentId": parseInt(*(currency).At(MkString("id")))})
	_ = request
	response := this.Call(MkString("privatePostGetDepositAddresses"), this.Extend(request, params))
	_ = response
	addresses := this.SafeValue(response, MkString("addresses"), MkArray(&VarArray{}))
	_ = addresses
	address := this.SafeValue(addresses, MkInteger(0))
	_ = address
...
func (this *Eqonex) ParseDepositAddress(goArgs ...*Variant) *Variant {
	...
	_ = currency
	currencyId := this.SafeString(depositAddress, MkString("symbol"))
	_ = currencyId
...
func (this *Eqonex) FetchDeposits(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privatePostGetDepositHistory"), this.Extend(request, params))
	_ = response
...
func (this *Eqonex) FetchWithdrawals(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privatePostGetWithdrawRequests"), this.Extend(request, params))
	_ = response
	withdrawals := this.SafeValue(response, MkString("addresses"), MkArray(&VarArray{}))
	_ = withdrawals
...
func (this *Eqonex) ParseTransaction(goArgs ...*Variant) *Variant {
	...
	currency := GoGetArg(goArgs, 1, MkUndefined())
	_ = currency
	id := this.SafeString(transaction, MkString("id"), MkString("transactionId"))
	_ = id
	txid := this.SafeString(transaction, MkString("transactionUuid"))
	_ = txid
	...
	_ = type_
	amount := this.SafeNumber(transaction, MkString("balance_change"))
	_ = amount
	if IsTrue(OpEq2(amount, MkUndefined())) {
		amount = this.SafeString(transaction, MkString("quantity"))
		amountScale := this.SafeInteger(transaction, MkString("quantity_scale"))
		_ = amountScale
...
func (this *Eqonex) Withdraw(goArgs ...*Variant) *Variant {
	...
	currency := this.Currency(code)
	_ = currency
	scale := this.GetScale(amount)
	_ = scale
	quantity := this.ConvertToScale(amount, scale)
	_ = quantity
	request := MkMap(&VarMap{
	...
	_ = request
	response := this.Call(MkString("privatePostSendWithdrawRequest"), this.Extend(request, params))
	_ = response
...
func (this *Eqonex) FetchTradingFees(goArgs ...*Variant) *Variant {
	params := GoGetArg(goArgs, 0, MkMap(&VarMap{}))
	_ = params
	response := this.Call(MkString("publicGetGetExchangeInfo"), params)
	_ = response
	tradingFees := this.SafeValue(response, MkString("spotFees"), MkArray(&VarArray{}))
	_ = tradingFees
	taker := MkMap(&VarMap{})
	_ = taker
	maker := MkMap(&VarMap{})
...
func (this *Eqonex) FetchTradingLimits(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("publicGetGetExchangeInfo"), params)
	_ = response
	tradingLimits := this.SafeValue(response, MkString("tradingLimits"), MkArray(&VarArray{}))
	_ = tradingLimits
	return MkMap(&VarMap{
		"info": tradingLimits,
		"limits": MkMap(&VarMap{
...
func (this *Eqonex) FetchFundingLimits(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("publicGetGetExchangeInfo"), params)
	_ = response
	withdrawLimits := this.SafeValue(response, MkString("withdrawLimits"), MkArray(&VarArray{}))
	_ = withdrawLimits
	return MkMap(&VarMap{
		"info":     withdrawLimits,
...
func (this *Eqonex) ParseOrder(goArgs ...*Variant) *Variant {
	...
	market := GoGetArg(goArgs, 1, MkUndefined())
	_ = market
	id := this.SafeString2(order, MkString("orderId"), MkString("id"))
	_ = id
	id = this.SafeString(order, MkString("origOrderId"), id)
	clientOrderId := this.SafeString(order, MkString("clOrdId"))
	_ = clientOrderId
	type_ := this.ParseOrderType(this.SafeString(order, MkString("ordType")))
	_ = type_
	side := this.ParseOrderSide(this.SafeString(order, MkString("side")))
	_ = side
	status := this.ParseOrderStatus(this.SafeString(order, MkString("ordStatus")))
	_ = status
	marketId := this.SafeString(order, MkString("instrumentId"))
	_ = marketId
	symbol := this.SafeSymbol(marketId, market)
	_ = symbol
	timestamp := this.ToMilliseconds(this.SafeString(order, MkString("timeStamp")))
	_ = timestamp
	lastTradeTimestamp := OpCopy(MkUndefined())
	_ = lastTradeTimestamp
	priceString := this.SafeString(order, MkString("price"))
	_ = priceString
	priceScale := this.SafeInteger(order, MkString("price_scale"))
	_ = priceScale
	price := this.ParseNumber(this.ConvertFromScale(priceString, priceScale))
	_ = price
	amountString := this.SafeString(order, MkString("quantity"))
	_ = amountString
	amountScale := this.SafeInteger(order, MkString("quantity_scale"))
	_ = amountScale
	amount := this.ParseNumber(this.ConvertFromScale(amountString, amountScale))
	_ = amount
	filledString := this.SafeString(order, MkString("cumQty"))
	_ = filledString
	filledScale := this.SafeInteger(order, MkString("cumQty_scale"))
	_ = filledScale
	filled := this.ParseNumber(this.ConvertFromScale(filledString, filledScale))
	_ = filled
	remainingString := this.SafeString(order, MkString("leavesQty"))
	_ = remainingString
	remainingScale := this.SafeInteger(order, MkString("leavesQty_scale"))
	_ = remainingScale
	remaining := this.ParseNumber(this.ConvertFromScale(remainingString, remainingScale))
	_ = remaining
	fee := OpCopy(MkUndefined())
	_ = fee
	currencyId := this.SafeInteger(order, MkString("feeInstrumentId"))
	_ = currencyId
	feeCurrencyCode := this.SafeCurrencyCode(currencyId)
	_ = feeCurrencyCode
	feeCost := this.SafeString(order, MkString("feeTotal"))
	_ = feeCost
	feeScale := this.SafeInteger(order, MkString("fee_scale"))
	_ = feeScale
	if IsTrue(OpNotEq2(feeCost, MkUndefined())) {
	...
	stopPriceScale := this.SafeInteger(order, MkString("stopPx_scale"), MkInteger(0))
	_ = stopPriceScale
	stopPrice := this.ParseNumber(this.ConvertFromScale(this.SafeString(order, MkString("stopPx")), stopPriceScale))
	_ = stopPrice
...
func (this *Eqonex) ConvertFromScale(goArgs ...*Variant) *Variant {
	number := GoGetArg(goArgs, 0, MkUndefined())
	_ = number
...
func (this *Eqonex) GetScale(goArgs ...*Variant) *Variant {
	num := GoGetArg(goArgs, 0, MkUndefined())
	_ = num
	s := this.NumberToString(num)
	_ = s
...
func (this *Eqonex) ConvertToScale(goArgs ...*Variant) *Variant {
	...
	precise.Reduce()
	preciseString := precise.ToString()
	_ = preciseString
...
func (this *Eqonex) Sign(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpEq2(api, MkString("public"))) {
		...
		if IsTrue(OpEq2(api, MkString("private"))) {
			format := this.SafeValue(params, MkString("format"))
			_ = format
			type_ := this.SafeValue(params, MkString("type"))
			_ = type_
			extension := MkMap(&VarMap{})
			_ = extension
			...
			*(params).At(MkString("nonce")) = this.Nonce()
			...
			signature := this.Hmac(this.Encode(body), this.Encode(*this.At(MkString("secret"))), MkString("sha384"))
			_ = signature
