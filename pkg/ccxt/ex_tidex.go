...
func (this *Tidex) FetchCurrencies(goArgs ...*Variant) *Variant {
	...
	_ = params
	response := this.Call(MkString("webGetCurrency"), params)
	_ = response
	result := MkMap(&VarMap{})
	_ = result
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
		...
		_ = id
		precision := this.SafeInteger(currency, MkString("amountPoint"))
		_ = precision
		code := this.SafeCurrencyCode(id)
		_ = code
		visible := this.SafeValue(currency, MkString("visible"))
		_ = visible
		active := OpEq2(visible, MkBool(true))
		_ = active
		withdrawEnable := this.SafeValue(currency, MkString("withdrawEnable"))
		_ = withdrawEnable
		depositEnable := this.SafeValue(currency, MkString("depositEnable"))
		_ = depositEnable
		canWithdraw := OpEq2(withdrawEnable, MkBool(true))
		_ = canWithdraw
		canDeposit := OpEq2(depositEnable, MkBool(true))
		_ = canDeposit
...
func (this *Tidex) FetchMarkets(goArgs ...*Variant) *Variant {
	params := GoGetArg(goArgs, 0, MkMap(&VarMap{}))
	_ = params
	response := this.Call(MkString("publicGetInfo"), params)
	_ = response
	markets := *(response).At(MkString("pairs"))
	_ = markets
	...
	for i := MkInteger(0); IsTrue(OpLw(i, keys.Length)); OpIncr(&i) {
		...
		_ = limits
		hidden := this.SafeInteger(market, MkString("hidden"))
		_ = hidden
		active := OpEq2(hidden, MkInteger(0))
		_ = active
		takerFeeString := this.SafeString(market, MkString("fee"))
		_ = takerFeeString
		takerFeeString = Precise.StringDiv(takerFeeString, MkString("100"))
		takerFee := this.ParseNumber(takerFeeString)
		_ = takerFee
...
func (this *Tidex) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("privatePostGetInfoExt"), params)
	_ = response
	balances := this.SafeValue(response, MkString("return"))
	_ = balances
	timestamp := this.SafeTimestamp(balances, MkString("server_time"))
	_ = timestamp
	result := MkMap(&VarMap{
		"info":      response,
		"timestamp": timestamp,
	...
	for i := MkInteger(0); IsTrue(OpLw(i, currencyIds.Length)); OpIncr(&i) {
		...
		_ = code
		balance := this.SafeValue(funds, currencyId, MkMap(&VarMap{}))
		_ = balance
...
func (this *Tidex) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	_ = response
	market_id_in_reponse := OpHasMember(*(market).At(MkString("id")), response)
	_ = market_id_in_reponse
	...
	orderbook := *(response).At(*(market).At(MkString("id")))
	_ = orderbook
...
func (this *Tidex) FetchTickers(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	ids := OpCopy(*this.At(MkString("ids")))
	_ = ids
	if IsTrue(OpEq2(symbols, MkUndefined())) {
		numIds := OpCopy(ids.Length)
		_ = numIds
		ids = ids.Join(MkString("-"))
		if IsTrue(OpGt(ids.Length, *(*this.At(MkString("options"))).At(MkString("fetchTickersMaxLength")))) {
			maxLength := this.SafeInteger(*this.At(MkString("options")), MkString("fetchTickersMaxLength"), MkInteger(2048))
			_ = maxLength
	...
	request := MkMap(&VarMap{"pair": ids})
	_ = request
	response := this.Call(MkString("publicGetTickerPair"), this.Extend(request, params))
	_ = response
	result := MkMap(&VarMap{})
	_ = result
...
func (this *Tidex) ParseTrade(goArgs ...*Variant) *Variant {
	...
	priceString := this.SafeString2(trade, MkString("rate"), MkString("price"))
	_ = priceString
	id := this.SafeString2(trade, MkString("trade_id"), MkString("tid"))
	_ = id
	...
	if IsTrue(OpNotEq2(feeCost, MkUndefined())) {
		feeCurrencyId := this.SafeString(trade, MkString("commissionCurrency"))
		_ = feeCurrencyId
	...
	isYourOrder := this.SafeValue(trade, MkString("is_your_order"))
	_ = isYourOrder
...
func (this *Tidex) FetchTrades(goArgs ...*Variant) *Variant {
	...
	if IsTrue(GoIsArray(response)) {
		numElements := OpCopy(response.Length)
		_ = numElements
...
func (this *Tidex) CreateOrder(goArgs ...*Variant) *Variant {
	...
	_ = remaining
	returnResult := this.SafeValue(response, MkString("return"))
	_ = returnResult
...
func (this *Tidex) ParseOrder(goArgs ...*Variant) *Variant {
	...
	_ = status
	timestamp := this.SafeTimestamp(order, MkString("timestamp_created"))
	_ = timestamp
...
func (this *Tidex) FetchOrder(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"order_id": parseInt(id)})
	_ = request
	response := this.Call(MkString("privatePostOrderInfo"), this.Extend(request, params))
	_ = response
	id = id.ToString()
	result := this.SafeValue(response, MkString("return"), MkMap(&VarMap{}))
	_ = result
	order := this.SafeValue(result, id)
	_ = order
...
func (this *Tidex) FetchOpenOrders(goArgs ...*Variant) *Variant {
	...
	_ = response
	orders := this.SafeValue(response, MkString("return"), MkArray(&VarArray{}))
	_ = orders
...
func (this *Tidex) FetchMyTrades(goArgs ...*Variant) *Variant {
	...
	_ = response
	trades := this.SafeValue(response, MkString("return"), MkArray(&VarArray{}))
	_ = trades
...
func (this *Tidex) Withdraw(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privatePostWithdrawCoin"), this.Extend(request, params))
	_ = response
	return MkMap(&VarMap{
		"info": response,
...
func (this *Tidex) HandleErrors(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpHasMember(MkString("success"), response)) {
		...
		if IsTrue(OpNot(success)) {
			...
			_ = code
			message := this.SafeString(response, MkString("error"))
			_ = message
