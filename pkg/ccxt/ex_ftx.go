...
func (this *Ftx) FetchCurrencies(goArgs ...*Variant) *Variant {
	...
	_ = params
	response := this.Call(MkString("publicGetCoins"), params)
	_ = response
...
func (this *Ftx) FetchMarkets(goArgs ...*Variant) *Variant {
	...
	for i := MkInteger(0); IsTrue(OpLw(i, markets.Length)); OpIncr(&i) {
		...
		_ = id
		baseId := this.SafeString2(market, MkString("baseCurrency"), MkString("underlying"))
		_ = baseId
		quoteId := this.SafeString(market, MkString("quoteCurrency"), MkString("USD"))
		_ = quoteId
		...
		_ = quote
		symbol := OpTrinary(OpEq2(type_, MkString("future")), this.SafeString(market, MkString("name")), OpAdd(base, OpAdd(MkString("/"), quote)))
		_ = symbol
		active := this.SafeValue(market, MkString("enabled"))
		_ = active
		sizeIncrement := this.SafeNumber(market, MkString("sizeIncrement"))
		_ = sizeIncrement
		priceIncrement := this.SafeNumber(market, MkString("priceIncrement"))
		_ = priceIncrement
...
func (this *Ftx) ParseTicker(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpHasMember(marketId, *this.At(MkString("markets_by_id")))) {
		...
		type_ := this.SafeString(ticker, MkString("type"))
		_ = type_
		if IsTrue(OpEq2(type_, MkString("future"))) {
			symbol = OpCopy(marketId)
		} else {
			base := this.SafeCurrencyCode(this.SafeString(ticker, MkString("baseCurrency")))
			_ = base
			quote := this.SafeCurrencyCode(this.SafeString(ticker, MkString("quoteCurrency")))
			_ = quote
	...
	_ = last
	timestamp := this.SafeTimestamp(ticker, MkString("time"), this.Milliseconds())
	_ = timestamp
	return MkMap(&VarMap{
		"symbol":        symbol,
		"timestamp":     timestamp,
...
func (this *Ftx) FetchTicker(goArgs ...*Variant) *Variant {
	...
	_ = market
	request := MkMap(&VarMap{"market_name": *(market).At(MkString("id"))})
	_ = request
	response := this.Call(MkString("publicGetMarketsMarketName"), this.Extend(request, params))
	_ = response
...
func (this *Ftx) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("publicGetMarketsMarketNameOrderbook"), this.Extend(request, params))
	_ = response
	result := this.SafeValue(response, MkString("result"), MkMap(&VarMap{}))
	_ = result
...
func (this *Ftx) GetMarketId(goArgs ...*Variant) *Variant {
	...
	params := GoGetArg(goArgs, 2, MkMap(&VarMap{}))
	_ = params
	parts := this.GetMarketParams(symbol, key, params)
	_ = parts
...
func (this *Ftx) FetchOHLCV(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("publicGetMarketsMarketNameCandles"), this.Extend(request, params))
	_ = response
...
func (this *Ftx) ParseTrade(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpHasMember(marketId, *this.At(MkString("markets_by_id")))) {
		...
		base := this.SafeCurrencyCode(this.SafeString(trade, MkString("baseCurrency")))
		_ = base
		quote := this.SafeCurrencyCode(this.SafeString(trade, MkString("quoteCurrency")))
		_ = quote
		if IsTrue(OpAnd(OpNotEq2(base, MkUndefined()), OpNotEq2(quote, MkUndefined()))) {
			symbol = OpAdd(base, OpAdd(MkString("/"), quote))
		} else {
...
func (this *Ftx) FetchTrades(goArgs ...*Variant) *Variant {
	...
	_ = marketId
	ArrayUnpack(this.GetMarketParams(symbol, MkString("market_name"), params), &market, &marketId)
	request := MkMap(&VarMap{"market_name": marketId})
	_ = request
	...
	response := this.Call(MkString("publicGetMarketsMarketNameTrades"), this.Extend(request, params))
	_ = response
	result := this.SafeValue(response, MkString("result"), MkArray(&VarArray{}))
	_ = result
...
func (this *Ftx) FetchBalance(goArgs ...*Variant) *Variant {
	...
	for i := MkInteger(0); IsTrue(OpLw(i, balances.Length)); OpIncr(&i) {
		...
		_ = balance
		code := this.SafeCurrencyCode(this.SafeString(balance, MkString("coin")))
		_ = code
...
func (this *Ftx) ParseOrder(goArgs ...*Variant) *Variant {
	...
	_ = amount
	filled := this.SafeNumber(order, MkString("filledSize"))
	_ = filled
	remaining := this.SafeNumber(order, MkString("remainingSize"))
	_ = remaining
	...
	_ = type_
	average := this.SafeNumber(order, MkString("avgFillPrice"))
	_ = average
	price := this.SafeNumber2(order, MkString("price"), MkString("triggerPrice"), average)
	_ = price
	...
	lastTradeTimestamp := this.Parse8601(this.SafeString(order, MkString("triggeredAt")))
	_ = lastTradeTimestamp
	clientOrderId := this.SafeString(order, MkString("clientId"))
	_ = clientOrderId
...
func (this *Ftx) CreateOrder(goArgs ...*Variant) *Variant {
	...
	_ = request
	clientOrderId := this.SafeString2(params, MkString("clientId"), MkString("clientOrderId"))
	_ = clientOrderId
		...
		if IsTrue(OpEq2(type_, MkString("market"))) {
			...
			*(request).At(MkString("price")) = OpCopy(MkUndefined())
			...
			if IsTrue(OpOr(OpEq2(type_, MkString("stop")), OpEq2(type_, MkString("takeProfit")))) {
				...
				stopPrice := this.SafeNumber2(params, MkString("stopPrice"), MkString("triggerPrice"))
				_ = stopPrice
...
func (this *Ftx) EditOrder(goArgs ...*Variant) *Variant {
	...
	method := OpCopy(MkUndefined())
	_ = method
	clientOrderId := this.SafeString2(params, MkString("client_order_id"), MkString("clientOrderId"))
	_ = clientOrderId
	triggerPrice := this.SafeNumber(params, MkString("triggerPrice"))
	_ = triggerPrice
	orderPrice := this.SafeNumber(params, MkString("orderPrice"))
	_ = orderPrice
	trailValue := this.SafeNumber(params, MkString("trailValue"))
	_ = trailValue
	params = this.Omit(params, MkArray(&VarArray{
	...
	triggerPriceIsDefined := OpNotEq2(triggerPrice, MkUndefined())
	_ = triggerPriceIsDefined
	orderPriceIsDefined := OpNotEq2(orderPrice, MkUndefined())
	_ = orderPriceIsDefined
	trailValueIsDefined := OpNotEq2(trailValue, MkUndefined())
	_ = trailValueIsDefined
...
func (this *Ftx) CancelOrder(goArgs ...*Variant) *Variant {
	...
	_ = options
	defaultMethod := this.SafeString(options, MkString("method"), MkString("privateDeleteOrdersOrderId"))
	_ = defaultMethod
	method := this.SafeString(params, MkString("method"), defaultMethod)
	_ = method
	type_ := this.SafeValue(params, MkString("type"))
	_ = type_
	clientOrderId := this.SafeValue2(params, MkString("client_order_id"), MkString("clientOrderId"))
	_ = clientOrderId
	if IsTrue(OpEq2(clientOrderId, MkUndefined())) {
...
func (this *Ftx) CancelAllOrders(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{})
	_ = request
	marketId := this.GetMarketId(symbol, MkString("market"), params)
	_ = marketId
	if IsTrue(OpNotEq2(marketId, MkUndefined())) {
...
func (this *Ftx) FetchOpenOrders(goArgs ...*Variant) *Variant {
	...
	_ = options
	defaultMethod := this.SafeString(options, MkString("method"), MkString("privateGetOrders"))
	_ = defaultMethod
	method := this.SafeString(params, MkString("method"), defaultMethod)
	_ = method
	type_ := this.SafeValue(params, MkString("type"))
...
func (this *Ftx) FetchOrders(goArgs ...*Variant) *Variant {
	...
	_ = options
	defaultMethod := this.SafeString(options, MkString("method"), MkString("privateGetOrdersHistory"))
	_ = defaultMethod
	method := this.SafeString(params, MkString("method"), defaultMethod)
	_ = method
	type_ := this.SafeValue(params, MkString("type"))
...
func (this *Ftx) FetchMyTrades(goArgs ...*Variant) *Variant {
	...
	_ = response
	trades := this.SafeValue(response, MkString("result"), MkArray(&VarArray{}))
	_ = trades
...
func (this *Ftx) Withdraw(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privatePostWalletWithdrawals"), this.Extend(request, params))
	_ = response
	result := this.SafeValue(response, MkString("result"), MkMap(&VarMap{}))
	_ = result
...
func (this *Ftx) FetchDepositAddress(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privateGetWalletDepositAddressCoin"), this.Extend(request, params))
	_ = response
	result := this.SafeValue(response, MkString("result"), MkMap(&VarMap{}))
	_ = result
	address := this.SafeString(result, MkString("address"))
	_ = address
	tag := this.SafeString(result, MkString("tag"))
	_ = tag
	this.CheckAddress(address)
	return MkMap(&VarMap{
		"currency": code,
...
func (this *Ftx) ParseTransaction(goArgs ...*Variant) *Variant {
	...
	_ = currency
	code := this.SafeCurrencyCode(this.SafeString(transaction, MkString("coin")))
	_ = code
	id := this.SafeString(transaction, MkString("id"))
	_ = id
	amount := this.SafeNumber(transaction, MkString("size"))
	_ = amount
	...
	_ = tag
	address := this.SafeValue(transaction, MkString("address"))
	_ = address
	...
	if IsTrue(OpEq2(address, MkUndefined())) {
		notes := this.SafeString(transaction, MkString("notes"))
		_ = notes
...
func (this *Ftx) FetchDeposits(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("privateGetWalletDeposits"), params)
	_ = response
	result := this.SafeValue(response, MkString("result"), MkArray(&VarArray{}))
	_ = result
...
func (this *Ftx) FetchWithdrawals(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("privateGetWalletWithdrawals"), params)
	_ = response
	result := this.SafeValue(response, MkString("result"), MkArray(&VarArray{}))
	_ = result
	currency := OpCopy(MkUndefined())
...
func (this *Ftx) Sign(goArgs ...*Variant) *Variant {
	...
	_ = body
	request := OpAdd(MkString("/api/"), this.ImplodeParams(path, params))
	_ = request
	...
	if IsTrue(OpEq2(api, MkString("private"))) {
		...
		_ = signature
		options := this.SafeValue(*this.At(MkString("options")), MkString("sign"), MkMap(&VarMap{}))
		_ = options
		headerPrefix := this.SafeString(options, *this.At(MkString("hostname")), MkString("FTX"))
		_ = headerPrefix
		keyField := OpAdd(headerPrefix, MkString("-KEY"))
		_ = keyField
		tsField := OpAdd(headerPrefix, MkString("-TS"))
		_ = tsField
		signField := OpAdd(headerPrefix, MkString("-SIGN"))
		_ = signField
...
func (this *Ftx) SetLeverage(goArgs ...*Variant) *Variant {
	...
	request := MkMap(&VarMap{"leverage": leverage})
	_ = request
...
func (this *Ftx) ParseIncome(goArgs ...*Variant) *Variant {
	...
	market := GoGetArg(goArgs, 1, MkUndefined())
	_ = market
	marketId := this.SafeString(income, MkString("future"))
	_ = marketId
	symbol := this.SafeSymbol(marketId, market)
	_ = symbol
	amount := this.SafeNumber(income, MkString("payment"))
	_ = amount
	code := this.SafeCurrencyCode(MkString("USD"))
	_ = code
	id := this.SafeString(income, MkString("id"))
	_ = id
	timestamp := this.SafeInteger(income, MkString("time"))
	_ = timestamp
	rate := this.Call(MkString("safe_number"), income, MkString("rate"))
	_ = rate
	return MkMap(&VarMap{
		"info":      income,
		"symbol":    symbol,
...
func (this *Ftx) FetchFundingHistory(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	method := MkString("private_get_funding_payments")
	_ = method
	request := MkMap(&VarMap{})
	_ = request
	market := OpCopy(MkUndefined())
