...
func (this *Zb) FetchMarkets(goArgs ...*Variant) *Variant {
	...
	_ = params
	markets := this.Call(MkString("publicGetMarkets"), params)
	_ = markets
...
func (this *Zb) FetchCurrencies(goArgs ...*Variant) *Variant {
	params := GoGetArg(goArgs, 0, MkMap(&VarMap{}))
	_ = params
	response := this.Call(MkString("tradeGetGetFeeInfo"), params)
	_ = response
	currencies := this.SafeValue(response, MkString("result"), MkMap(&VarMap{}))
	_ = currencies
	...
	for i := MkInteger(0); IsTrue(OpLw(i, ids.Length)); OpIncr(&i) {
		...
		for j := MkInteger(0); IsTrue(OpLw(j, currency.Length)); OpIncr(&j) {
			networkItem := *(currency).At(j)
			_ = networkItem
			network := this.SafeString(networkItem, MkString("chainName"))
			_ = network
			withdrawFee := this.SafeNumber(networkItem, MkString("fee"))
			_ = withdrawFee
			depositEnable := this.SafeValue(networkItem, MkString("canDeposit"))
			_ = depositEnable
			withdrawEnable := this.SafeValue(networkItem, MkString("canWithdraw"))
			_ = withdrawEnable
...
func (this *Zb) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("privateGetGetAccountInfo"), params)
	_ = response
	balances := this.SafeValue(*(response).At(MkString("result")), MkString("coins"))
	_ = balances
	result := MkMap(&VarMap{
		"info":      response,
	...
	for i := MkInteger(0); IsTrue(OpLw(i, balances.Length)); OpIncr(&i) {
		...
		_ = account
		currencyId := this.SafeString(balance, MkString("key"))
		_ = currencyId
...
func (this *Zb) ParseDepositAddress(goArgs ...*Variant) *Variant {
	...
	_ = currency
	address := this.SafeString2(depositAddress, MkString("key"), MkString("address"))
	_ = address
	tag := OpCopy(MkUndefined())
	_ = tag
	memo := this.SafeString(depositAddress, MkString("memo"))
	_ = memo
	if IsTrue(OpNotEq2(memo, MkUndefined())) {
		tag = OpCopy(memo)
	} else {
		if IsTrue(OpGtEq(address.IndexOf(MkString("_")), MkInteger(0))) {
			parts := address.Split(MkString("_"))
			_ = parts
			address = *(parts).At(MkInteger(0))
			tag = *(parts).At(MkInteger(1))
		}
	}
	currencyId := this.SafeString(depositAddress, MkString("blockChain"))
	_ = currencyId
...
func (this *Zb) FetchDepositAddresses(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("privateGetGetPayinAddress"), params)
	_ = response
	message := this.SafeValue(response, MkString("message"), MkMap(&VarMap{}))
	_ = message
	datas := this.SafeValue(message, MkString("datas"), MkArray(&VarArray{}))
	_ = datas
...
func (this *Zb) FetchDepositAddress(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privateGetGetUserAddress"), this.Extend(request, params))
	_ = response
	message := this.SafeValue(response, MkString("message"), MkMap(&VarMap{}))
	_ = message
	datas := this.SafeValue(message, MkString("datas"), MkMap(&VarMap{}))
	_ = datas
...
func (this *Zb) FetchTickers(goArgs ...*Variant) *Variant {
	...
	_ = params
	this.LoadMarkets()
	response := this.Call(MkString("publicGetAllTicker"), params)
	_ = response
	result := MkMap(&VarMap{})
	_ = result
	marketsByIdWithoutUnderscore := MkMap(&VarMap{})
	_ = marketsByIdWithoutUnderscore
	marketIds := GoGetKeys(*this.At(MkString("markets_by_id")))
	_ = marketIds
	for i := MkInteger(0); IsTrue(OpLw(i, marketIds.Length)); OpIncr(&i) {
		tickerId := (*(marketIds).At(i)).Call(MkString("replace"), MkString("_"), MkString(""))
		_ = tickerId
	...
	_ = ids
	for i := MkInteger(0); IsTrue(OpLw(i, ids.Length)); OpIncr(&i) {
		market := *(marketsByIdWithoutUnderscore).At(*(ids).At(i))
		_ = market
...
func (this *Zb) FetchTicker(goArgs ...*Variant) *Variant {
	...
	_ = response
	ticker := this.SafeValue(response, MkString("ticker"), MkMap(&VarMap{}))
	_ = ticker
...
func (this *Zb) ParseTicker(goArgs ...*Variant) *Variant {
	...
	_ = market
	timestamp := this.SafeInteger(ticker, MkString("date"), this.Milliseconds())
	_ = timestamp
...
func (this *Zb) FetchOrders(goArgs ...*Variant) *Variant {
	...
	_ = request
	method := MkString("privateGetGetOrdersIgnoreTradeType")
	_ = method
...
func (this *Zb) FetchClosedOrders(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privateGetGetFinishedAndPartialOrders"), this.Extend(request, params))
	_ = response
	return this.ParseOrders(response, market, since, limit)
}
...
func (this *Zb) FetchOpenOrders(goArgs ...*Variant) *Variant {
	...
	since := GoGetArg(goArgs, 1, MkUndefined())
	_ = since
	limit := GoGetArg(goArgs, 2, MkInteger(10))
	_ = limit
	params := GoGetArg(goArgs, 3, MkMap(&VarMap{}))
	_ = params
	if IsTrue(OpEq2(symbol, MkUndefined())) {
	...
	_ = request
	method := MkString("privateGetGetUnfinishedOrdersIgnoreTradeType")
	_ = method
	if IsTrue(OpHasMember(MkString("tradeType"), params)) {
		method = MkString("privateGetGetOrdersNew")
	}
...
func (this *Zb) ParseOrder(goArgs ...*Variant) *Variant {
	...
	market := GoGetArg(goArgs, 1, MkUndefined())
	_ = market
	side := this.SafeInteger(order, MkString("type"))
	_ = side
	side = OpTrinary(OpEq2(side, MkInteger(1)), MkString("buy"), MkString("sell"))
	type_ := MkString("limit")
	_ = type_
	timestamp := this.SafeInteger(order, MkString("trade_date"))
	_ = timestamp
	marketId := this.SafeString(order, MkString("currency"))
	_ = marketId
	symbol := this.SafeSymbol(marketId, market, MkString("_"))
	_ = symbol
	price := this.SafeNumber(order, MkString("price"))
	_ = price
	filled := this.SafeNumber(order, MkString("trade_amount"))
	_ = filled
	...
	_ = id
	feeCost := this.SafeNumber(order, MkString("fees"))
	_ = feeCost
	...
	if IsTrue(OpNotEq2(feeCost, MkUndefined())) {
		...
		_ = feeCurrency
		zbFees := this.SafeValue(order, MkString("useZbFee"))
		_ = zbFees
...
func (this *Zb) ParseTransaction(goArgs ...*Variant) *Variant {
	...
	_ = amount
	timestamp := this.Parse8601(this.SafeString(transaction, MkString("submit_time")))
	_ = timestamp
	timestamp = this.SafeInteger(transaction, MkString("submitTime"), timestamp)
	address := this.SafeString2(transaction, MkString("toAddress"), MkString("address"))
	_ = address
	tag := OpCopy(MkUndefined())
	_ = tag
	...
	confirmTimes := this.SafeInteger(transaction, MkString("confirmTimes"))
	_ = confirmTimes
	updated := this.SafeInteger(transaction, MkString("manageTime"))
	_ = updated
...
func (this *Zb) Withdraw(goArgs ...*Variant) *Variant {
	...
	params := GoGetArg(goArgs, 4, MkMap(&VarMap{}))
	_ = params
	password := this.SafeString(params, MkString("safePwd"), *this.At(MkString("password")))
	_ = password
	...
	fees := this.SafeNumber(params, MkString("fees"))
	_ = fees
	...
	_ = response
	transaction := this.ParseTransaction(response, currency)
	_ = transaction
...
func (this *Zb) FetchWithdrawals(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetGetWithdrawRecord"), this.Extend(request, params))
	_ = response
	message := this.SafeValue(response, MkString("message"), MkMap(&VarMap{}))
	_ = message
	datas := this.SafeValue(message, MkString("datas"), MkMap(&VarMap{}))
	_ = datas
	withdrawals := this.SafeValue(datas, MkString("list"), MkArray(&VarArray{}))
	_ = withdrawals
...
func (this *Zb) FetchDeposits(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetGetChargeRecord"), this.Extend(request, params))
	_ = response
	message := this.SafeValue(response, MkString("message"), MkMap(&VarMap{}))
	_ = message
	datas := this.SafeValue(message, MkString("datas"), MkMap(&VarMap{}))
	_ = datas
	deposits := this.SafeValue(datas, MkString("list"), MkArray(&VarArray{}))
	_ = deposits
...
func (this *Zb) Sign(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpEq2(api, MkString("public"))) {
		...
		if IsTrue(OpEq2(api, MkString("trade"))) {
			...
			_ = auth
			secret := this.Hash(this.Encode(*this.At(MkString("secret"))), MkString("sha1"))
			_ = secret
			signature := this.Hmac(this.Encode(auth), this.Encode(secret), MkString("md5"))
			_ = signature
			suffix := OpAdd(MkString("sign="), OpAdd(signature, OpAdd(MkString("&reqTime="), nonce.ToString())))
			_ = suffix
