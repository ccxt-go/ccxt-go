...
func (this *Liquid) FetchCurrencies(goArgs ...*Variant) *Variant {
	...
	for i := MkInteger(0); IsTrue(OpLw(i, response.Length)); OpIncr(&i) {
		...
		_ = name
		active := OpAnd(*(currency).At(MkString("depositable")), *(currency).At(MkString("withdrawable")))
		_ = active
		amountPrecision := this.SafeInteger(currency, MkString("assets_precision"))
		_ = amountPrecision
...
func (this *Liquid) FetchMarkets(goArgs ...*Variant) *Variant {
	params := GoGetArg(goArgs, 0, MkMap(&VarMap{}))
	_ = params
	spot := this.Call(MkString("publicGetProducts"), params)
	_ = spot
	perpetual := this.Call(MkString("publicGetProducts"), MkMap(&VarMap{"perpetual": MkString("1")}))
	_ = perpetual
	currencies := this.FetchCurrencies()
	_ = currencies
	currenciesByCode := this.IndexBy(currencies, MkString("code"))
	_ = currenciesByCode
	result := MkArray(&VarArray{})
	_ = result
	markets := this.ArrayConcat(spot, perpetual)
	_ = markets
	for i := MkInteger(0); IsTrue(OpLw(i, markets.Length)); OpIncr(&i) {
		...
		_ = baseId
		quoteId := this.SafeString(market, MkString("quoted_currency"))
		_ = quoteId
		productType := this.SafeString(market, MkString("product_type"))
		_ = productType
		type_ := MkString("spot")
		_ = type_
		...
		disabled := this.SafeValue(market, MkString("disabled"), MkBool(false))
		_ = disabled
		active := OpNot(disabled)
		_ = active
		baseCurrency := this.SafeValue(currenciesByCode, base)
		_ = baseCurrency
		...
		lastPrice := this.SafeNumber(market, MkString("last_traded_price"))
		_ = lastPrice
		...
		_ = maxPrice
		if IsTrue(lastPrice) {
			multiplierDown := this.SafeNumber(market, MkString("multiplier_down"))
			_ = multiplierDown
			multiplierUp := this.SafeNumber(market, MkString("multiplier_up"))
			_ = multiplierUp
...
func (this *Liquid) FetchBalance(goArgs ...*Variant) *Variant {
	...
	_ = result
	crypto := this.SafeValue(response, MkString("crypto_accounts"), MkArray(&VarArray{}))
	_ = crypto
	fiat := this.SafeValue(response, MkString("fiat_accounts"), MkArray(&VarArray{}))
	_ = fiat
...
func (this *Liquid) FetchOrderBook(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("publicGetProductsIdPriceLevels"), this.Extend(request, params))
	_ = response
...
func (this *Liquid) ParseTicker(goArgs ...*Variant) *Variant {
	...
	if IsTrue(OpEq2(market, MkUndefined())) {
		marketId := this.SafeString(ticker, MkString("id"))
		_ = marketId
		if IsTrue(OpHasMember(marketId, *this.At(MkString("markets_by_id")))) {
			market = *(*this.At(MkString("markets_by_id"))).At(marketId)
		} else {
			baseId := this.SafeString(ticker, MkString("base_currency"))
			_ = baseId
			quoteId := this.SafeString(ticker, MkString("quoted_currency"))
			_ = quoteId
	...
	_ = average
	open := this.SafeNumber(ticker, MkString("last_price_24h"))
	_ = open
...
func (this *Liquid) FetchTicker(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("publicGetProductsId"), this.Extend(request, params))
	_ = response
...
func (this *Liquid) ParseTrade(goArgs ...*Variant) *Variant {
	...
	market := GoGetArg(goArgs, 1, MkUndefined())
	_ = market
	timestamp := this.SafeTimestamp(trade, MkString("created_at"))
	_ = timestamp
	orderId := this.SafeString(trade, MkString("order_id"))
	_ = orderId
	takerSide := this.SafeString(trade, MkString("taker_side"))
	_ = takerSide
	mySide := this.SafeString(trade, MkString("my_side"))
	_ = mySide
	side := OpTrinary(OpNotEq2(mySide, MkUndefined()), mySide, takerSide)
	_ = side
...
func (this *Liquid) FetchTrades(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("publicGetExecutions"), this.Extend(request, params))
	_ = response
	result := OpTrinary(OpNotEq2(since, MkUndefined()), response, *(response).At(MkString("models")))
	_ = result
...
func (this *Liquid) FetchMyTrades(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetExecutionsMe"), this.Extend(request, params))
	_ = response
...
func (this *Liquid) CancelOrder(goArgs ...*Variant) *Variant {
	...
	_ = request
	response := this.Call(MkString("privatePutOrdersIdCancel"), this.Extend(request, params))
	_ = response
...
func (this *Liquid) ParseOrder(goArgs ...*Variant) *Variant {
	...
	_ = type_
	tradeCost := MkInteger(0)
	_ = tradeCost
	tradeFilled := MkInteger(0)
	_ = tradeFilled
...
func (this *Liquid) FetchOrders(goArgs ...*Variant) *Variant {
	...
	_ = market
	request := MkMap(&VarMap{"with_details": MkInteger(1)})
	_ = request
	...
	_ = response
	orders := this.SafeValue(response, MkString("models"), MkArray(&VarArray{}))
	_ = orders
...
func (this *Liquid) FetchOpenOrders(goArgs ...*Variant) *Variant {
	...
	params := GoGetArg(goArgs, 3, MkMap(&VarMap{}))
	_ = params
	request := MkMap(&VarMap{"status": MkString("live")})
	_ = request
...
func (this *Liquid) FetchClosedOrders(goArgs ...*Variant) *Variant {
	...
	params := GoGetArg(goArgs, 3, MkMap(&VarMap{}))
	_ = params
	request := MkMap(&VarMap{"status": MkString("filled")})
	_ = request
	return this.FetchOrders(symbol, since, limit, this.Extend(request, params))
}
...
func (this *Liquid) Withdraw(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privatePostCryptoWithdrawals"), this.Extend(request, params))
	_ = response
...
func (this *Liquid) FetchWithdrawals(goArgs ...*Variant) *Variant {
	...
	response := this.Call(MkString("privateGetCryptoWithdrawals"), this.Extend(request, params))
	_ = response
	transactions := this.SafeValue(response, MkString("models"), MkArray(&VarArray{}))
	_ = transactions
...
func (this *Liquid) ParseTransaction(goArgs ...*Variant) *Variant {
	...
	_ = address
	tag := this.SafeString2(transaction, MkString("payment_id"), MkString("memo_value"))
	_ = tag
	txid := this.SafeString(transaction, MkString("transaction_hash"))
	_ = txid
	currencyId := this.SafeString2(transaction, MkString("currency"), MkString("asset"))
	_ = currencyId
	code := this.SafeCurrencyCode(currencyId, currency)
	_ = code
	timestamp := this.SafeTimestamp(transaction, MkString("created_at"))
	_ = timestamp
	updated := this.SafeTimestamp(transaction, MkString("updated_at"))
	_ = updated
	type_ := MkString("withdrawal")
	_ = type_
	status := this.ParseTransactionStatus(this.SafeString(transaction, MkString("state")))
	_ = status
	amountString := this.SafeString(transaction, MkString("amount"))
	_ = amountString
	feeCostString := this.SafeString(transaction, MkString("withdrawal_fee"))
	_ = feeCostString
	amount := this.ParseNumber(Precise.StringSub(amountString, feeCostString))
	_ = amount
	return MkMap(&VarMap{
		"info":      transaction,
		"id":        id,
